...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        // --- 1. WELCOME (RACE VS QUALI) ---
        .addSubcommand(sub =>
            sub.setName('welcome')
                .setDescription('Broadcast session welcome message in voice channel')
                .addStringOption(opt => 
                    opt.setName('session')
                        .setDescription('Select session type (Default: Race)')
                        .setRequired(false)
                        .addChoices(
                ...
                .addStringOption(opt => opt.setName('laps').setDescription('Duration or Laps (e.g. 25 minutes, 15 Laps)').setRequired(false))
        )
        ...
        .addSubcommand(sub =>
            sub.setName('penalty')
                ...
                .addStringOption(opt => opt.setName('location').setDescription('Turn / Corner (Default: turn 7)').setRequired(false))
                .addStringOption(opt => opt.setName('reason').setDescription('Reason (Default: taking too many liberties)').setRequired(false))
        )
